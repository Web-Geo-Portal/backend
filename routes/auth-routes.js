...
//     res.send({
//       status: 1, token: tokens,expiresIn:3600, message: "Login successfull",users
//     });
//   } catch (error) {
...
;
    ...
    const users = await pool.query('SELECT * FROM user_login WHERE user_email = $1', [email]);
    ...
    if(users.rows[0].blocked_time){
      ...
      if(time_left > 0 ){
        ...
        let emptyDate = null;
        await pool.query('UPDATE user_login set blocked_time = $2 WHERE user_email = $1', [email,emptyDate ]);
      }
    }
    //PASSWORD CHECK
    let validPassword = await bcrypt.compare(password, users.rows[0].user_password);
    if (!validPassword){
      ...
      if(attempts+1 > 3){
        let blockedDate = new Date(now.getTime() + 86400000)
        await pool.query('UPDATE user_login set blocked_time = $2 WHERE user_email = $1', [email,blockedDate ]);
        return res.send({status: 0,  error: "User blocked"});
      }
      return res.send({
        status: 2,  error: "Password Incorrect"
